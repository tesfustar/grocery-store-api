...
export const GetProductRequests = async (req: Request, res: Response) => {
  try {
    const requestedProduct = await ProductRequest.find().populate('product.product').populate('branch');;
...
export const GetProductRequestDetail = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const requestedDetail = await ProductRequest.findById(id).populate('product.product').populate('branch');;
